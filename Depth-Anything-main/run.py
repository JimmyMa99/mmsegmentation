...
#         # 更改保存的图像，仅保存深度图
#     filename = os.path.basename(filename)
#     cv2.imwrite(os.path.join(args.outdir, filename[:filename.rfind('.')] + '_depth.png'), depth_color)
import argparse
import cv2
import numpy as np
import os
...
if __name__ == '__main__':
    ...
    for filename in tqdm(filenames):
        ...
        output_path = os.path.join(args.outdir, output_filename[:output_filename.rfind('.')] + '_depth.png')
