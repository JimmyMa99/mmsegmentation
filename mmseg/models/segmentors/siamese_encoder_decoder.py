...
class SiameseEncoderDecoder(BaseSegmentor):
    ...
    def _decode_head_forward_train(self, inputs: List[Tensor],
                                   data_samples: SampleList,siamese: bool) -> dict:
        """Run forward function and calculate loss for decode head in
        training."""
        losses = dict()
        # pdb.set_trace()
        loss_decode,data_samples = self.decode_head.loss(inputs, data_samples,
                                            self.train_cfg,siamese=siamese)
        if siamese:
            loss_decode = {key: self.main_loss[key] + loss_decode[key] for key in self.main_loss.keys()}
        else:
            self.main_loss = loss_decode
        losses.update(add_prefix(loss_decode, 'decode'))
        return losses, data_samples
    ...
    def loss(self, inputs: Tensor, data_samples: SampleList) -> dict:
        ...
        # pdb.set_trace()
        siamese_inputs=inputs['siamese_inputs']
        inputs=inputs['inputs']
        x_inputs = self.extract_feat(inputs)
        # o = self.decode_head.forward(x)
    ...
    def _forward(self,
        ...
        x = self.extract_feat(siamese_inputs)
        siamese_o = self.decode_head.forward(x)
        # x1 = self.extract_feat(inputs1)
        return dict(outputs=o,siamese_outputs=siamese_o)
